...
class NearestStationFinder(Solver):
    def travel_time_secs_to_nearest_solution_station(self, origin, analysis_start_time, maximum_time,
                                                     ...
                                                     walking_coordinates, solution_stops):
    ...
    def travel_or_walk_time_secs_to_nearest_solution_station(self, origin, analysis_start_time, maximum_time,
        ...
        self._find_travel_or_walk_time_secs(origin, analysis_start_time)
        return self._best_known_time
    ...
    def _find_next_travel_or_walk_time_secs(self, departure_time, origin):
    ...
    def _find_next_travel_time_secs(self, departure_time, origin):
        self._initialize_travel_progress_dict(origin, departure_time)
        self._exp_queue = ExpansionQueue(1, STOP_JOIN_STRING)
        self._exp_queue.add(self._progress_dict.keys())
        while not self._exp_queue.is_empty():
            self._expand()

    def _find_travel_or_walk_time_secs(self, origin, analysis_start_time):
        departure_time = self._find_next_departure_time(origin, analysis_start_time)
        if departure_time is None:
            return

        while departure_time is not None:
            self._find_next_travel_or_walk_time_secs(departure_time, origin)
    ...
    def _initialize_travel_progress_dict(self, origin, earliest_departure_time):
        self._progress_dict = dict()
        departure_time = self._find_next_departure_time(origin, earliest_departure_time)
        if departure_time is None:
            return
