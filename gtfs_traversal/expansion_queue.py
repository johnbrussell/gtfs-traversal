class ExpansionQueue:
    ...
    def add_with_depth(self, original_locations, progresses):
        ...
        assert len(original_locations) == len(walk) + len(depth_based_non_walk) + len(start_non_endpoints) + \
            len(start_endpoints) + len(solution) + len(transfer_endpoint) + \
            len(solution_unvisited) + len(transfer_transfer)
